...
document.addEventListener("click", (e) => {
    ...
    // BOTÓN ATENDER
    if (e.target.classList.contains("waiting-atender")) {
        e.stopPropagation();
...
<button class="waiting-atender" data-id="${w.id}">🩺</button>
`;
...
function atenderWaiting(id){

let waiting = JSON.parse(localStorage.getItem("waiting")) || [];

const turno = waiting.find(w => w.id === id);

if(!turno) return;


// guardar paciente a abrir
localStorage.setItem("openPet", JSON.stringify({
   clienteIndex: Number(turno.clienteIndex),
   mascotaIndex: Number(turno.mascotaIndex)
}));
