...
function initNavbar() {
  ...
  const veterinariosLS = JSON.parse(localStorage.getItem("veterinarios")) || {};
  ...
  if(adminId && admins[adminId]){
    texto = "Admin: " + admins[adminId].nombre;
  }

  // si entra veterinario mostrar ambos
  if(role === "vet" && vetId && veterinariosLS[vetId]){
    const vet = veterinariosLS[vetId];
    texto += "<br>Vet: " + vet.nombre;
  }

  if(subtitle){
    subtitle.innerHTML = texto;
  }
  ...
  // 🔒 si es veterinario ocultar settings
  if(role === "vet"){
